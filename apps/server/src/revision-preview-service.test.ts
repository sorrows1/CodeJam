...
describe('RevisionPreviewService', () => {
  ...
  it('uses container preparation only for an accepted Vite profile and rejects symlink assets', async () => {
    ...
    const link = path.join(root, `preview-${created.session.id}`, 'link'); await symlink(path.join(root, 'secret'), link).catch(() => undefined); if (await lstat(link).then(() => true).catch(() => false)) await expect(service.asset(missionId, created.session.id, created.token, 'link')).rejects.toThrow('unavailable');
  });
