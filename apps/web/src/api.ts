...
export const api = {
  ...
  createAgentPreview: (id: string) => request<{ session: { id: string; agentId: string; workspaceHash: string; profile: string; contentPath: string; isolatedOrigin: boolean; expiresAt: string; previewDataHash: string | null } }>(`/api/agents/${id}/previews`, { method: "POST", body: "{}" }),
  stopAgentPreview: (id: string, sessionId: string) => request<void>(`/api/agents/${id}/previews/${sessionId}`, { method: "DELETE" }),
  confirmImpact: (id: string, admissionId: string, choice: "governed" | "nonvisual" | "cancel") => request<{ admission: PlaygroundImpactAdmission }>(`/api/agents/${id}/impact-admissions/${admissionId}/confirm`, { method: "POST", body: JSON.stringify({ choice }) }),
  ...
  createPreview: (missionId: string, body: { kind: "design"; revisionId: string } | { kind: "workspace"; revisionId: string; designRevisionId: string }) => request<{ session: { id: string; missionId: string; target: { kind: "design"; revisionId: string } | { kind: "workspace"; revisionId: string; designRevisionId: string }; profile: string; contentPath: string; isolatedOrigin: boolean; expiresAt: string; previewDataHash: string | null } }>(`/api/missions/${missionId}/previews`, { method: "POST", body: JSON.stringify(body) }),
